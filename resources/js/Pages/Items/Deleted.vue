                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mt-4">
                    <div class="p-6 bg-white border-b border-gray-200">
                        <div class="flex justify-between">
                            <h3 class="text-2xl">Deleted Items</h3>
                            <Link :href="route('admin.items.index')" class="w-auto flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">View Items</Link>
                                    ...
                                    <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                                        <table class="min-w-full divide-y divide-gray-300">
                                            ...
                                            <tr v-for="item in items" :key="item.id">
                                                ...
                                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{ item.deleted_at }}</td>
                                                <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                                                    <Link class="text-indigo-600 hover:text-indigo-900 cursor-pointer"
                                                        :href="route('admin.items.restore', item.id)">
                                                        Restore
                                                    </Link>
                                                </td>
                                            </tr>
